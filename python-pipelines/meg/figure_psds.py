...
from config_meg import subjects, fname
...
ga_psds = {
    cond1: np.mean([p['pasat_run1'] for p in psds], axis=0),
    cond2: np.mean([p['pasat_run2'] for p in psds], axis=0),
    cond3: np.mean([p['eyesopen'] for p in psds], axis=0),
    'freqs': psds[0]['freqs']
}
...
info = mne.pick_info(info, mne.pick_types(info, meg=True, eeg=False, eog=False))
times = [1, 2]
