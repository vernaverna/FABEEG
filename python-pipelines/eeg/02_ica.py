...
from config_eeg import get_all_fnames, task_from_fname, fname, eog_channel, tasks, runs
...
exclude = ['restEC']  # These don't have eye blinks.
all_fnames = zip(
    get_all_fnames(args.subject, kind='filt', tasks=tasks, runs=runs, exclude=exclude),
    get_all_fnames(args.subject, kind='ica', tasks=tasks, runs=runs, exclude=exclude),
    get_all_fnames(args.subject, kind='clean', tasks=tasks, runs=runs, exclude=exclude),
)
