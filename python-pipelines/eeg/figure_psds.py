...
from mne.viz import iter_topography, plot_topomap
from mpl_toolkits.axes_grid1 import make_axes_locatable
from mne.stats import permutation_cluster_test
from mne.channels import find_ch_adjacency
from mne.stats import combine_adjacency, spatio_temporal_cluster_test
from eeg.config_eeg import fname, f_bands, bandpower
...
age_df = pd.read_csv('eeg/ages.csv', index_col=1)
...
def auc_over_freqrange(PSD_df, freqs, freqrange=(12,15), relative=True):
    ...
    for name, cohort_df in cohrt_groups:
        ...
        AUC_recs.append(  list( zip(namevec, cohort_df['Age'].values, AUCs_n1, AUCs_n2, cohort_df['Sex'].values, cohort_df.index.values, ) ))            

    
    # make AUC df for later analysis
    AUC_records = sum(AUC_recs, []) #unlist  
    AUC_df = pd.DataFrame.from_records(AUC_records, columns=['Age group', 'Age', 'AUC N1', 'AUC N2', 'Sex', 'Subject'])
    
    AUC_df = AUC_df.dropna()
    
    # return the data in a long format 
    AUC_df_long = pd.melt(AUC_df, id_vars=['Subject', 'Sex', 'Age', 'Age group'],
...
def within_sleep_stability(psd_df, freqs, stage='N1', spatial = False, color='green'):
    ...
    if spatial:
        ...
        significant_cluster_inds = np.where(cluster_pv < 0.05)[0]
        if len(significant_cluster_inds) == 0:
            print("No significant clusters found.")
        else:
            for i_clu, clu_idx in enumerate(significant_cluster_inds):
                clu_mask = clusters[clu_idx]  # Boolean mask: (freqs × channels)
                ch_inds, freq_inds = np.where(clu_mask)
        
                freq_inds = np.unique(freq_inds)
                ch_inds = np.unique(ch_inds)
        
                # Compute F-map (average over significant freqs)
                f_map = F_obs[freq_inds, :].mean(axis=0)
                mask = np.zeros((len(f_map), 1), dtype=bool)
                mask[ch_inds, :] = True
        
                # Create an MNE EvokedArray object for plotting
                f_evoked = mne.EvokedArray(f_map[:, np.newaxis], raw.info, tmin=0)
                ...
                f_evoked.plot_topomap(
                    ...
                    axes=ax_topo,
                    cmap="Blues",
                    vlim=(np.min(f_map), np.max(f_map)),
                    show=False,
                    colorbar=False,
                ...
                # --- Create layout for colorbar and frequency plot ---
                divider = make_axes_locatable(ax_topo)
                # Add colorbar for topomap
                ax_colorbar = divider.append_axes("right", size="5%", pad=0.05)
                plt.colorbar(image, cax=ax_colorbar)
                ax_topo.set_xlabel(f"Cluster #{i_clu + 1} | p = {cluster_pv[clu_idx]:.3f}")
                ax_topo.set_title("Significant Sensors (Topomap)")
        
                # --- Add Frequency Spectrum plot (F-stat across freqs) ---
                ax_spec = divider.append_axes("right", size="300%", pad=1.2)
                title = f"Cluster #{i_clu + 1} | {len(ch_inds)} ch(s)"
                if len(ch_inds) > 1:
                    title += " (max over channels)"
                # Extract F-values over freqs for significant channels
                F_obs_plot = F_obs[:, ch_inds].max(axis=-1)
                F_obs_plot_sig = np.full_like(F_obs_plot, np.nan)
                F_obs_plot_sig[freq_inds] = F_obs_plot[freq_inds]
        
                # Plot all F-values (gray) + highlight cluster (orange)
                for f_image, cmap in zip([F_obs_plot, F_obs_plot_sig], ["gray", "autumn"]):
                    ax_spec.plot(freqs, f_image, color="black" if cmap == "gray" else "red", lw=2)
        
                ax_spec.set_xlabel("Frequency (Hz)")
                ax_spec.set_ylabel("F-statistic")
...
PSD_df = pd.read_pickle('eeg/PSD_dataframe_with_bandpower.pkl')
...
subj=subjects[185].replace('sub-', '') #365, 399, 179, 48, 449
...
for i, (name, group_df) in enumerate(grouped_psd_df):
    ...
    clusts, pvs, fig, F_obs = within_sleep_stability(group_df, freqs, stage=stage, spatial=True, color=colors[i])
    plt.show()
...
##################################################################################################
plt.style.use("seaborn-v0_8-poster")   # or "seaborn-v0_8-white", "seaborn-v0_8-poster"
...
model = smf.mixedlm('AUC ~ Sleep', data=AUC_df_long,
                groups="Subject", vc_formula={"Group": "0 + Group"}, re_formula="~Sleep")
result = model.fit()
print(result.summary())


# test also the non-linearity effect of the AUC in both cases (N1 and N2)
AUC_df2 = AUC_df_long[AUC_df_long['Sleep']=='N1']
model_n1 = smf.ols("AUC ~ Age + I(Age**2) + I(Age**3)", data=AUC_df2).fit()
#model_n1 = smf.ols("AUC ~ I(Age**2) + I(Age**3)", data=AUC_df2).fit()

print(model_n1.summary())
coeffs = model_n1.params

# grid of ages for plotting
age_grid = np.linspace(AUC_df2["Age"].min(), AUC_df2["Age"].max(), 200)
pred_data = pd.DataFrame({"Age": age_grid})

# predict with confidence intervals
predictions = model_n1.get_prediction(pred_data)
...
plt.scatter(AUC_df2["Age"], AUC_df2["AUC"], alpha=0.3, color=colors[2], label="Observed data")

# fitted curve
plt.plot(age_grid, pred_summary["mean"], color='firebrick', label="Fitted cubic")

# confidence interval
plt.fill_between(age_grid,
                 pred_summary["mean_ci_lower"],
                 pred_summary["mean_ci_upper"],
                 color='firebrick', alpha=0.2, label="95% CI")

def fmt(coef):
    """Return coefficient with sign and 3 decimals."""
    return f"{coef:+.3f}"

# Build formula using LaTeX math syntax
formula_latex = (
    f"$AUC = {coeffs['Intercept']:.2f}"
    f"{fmt(coeffs['Age'])}\\,Age"
    f"{fmt(coeffs['I(Age ** 2)'])}\\,Age^{{2}}"
    f"{fmt(coeffs['I(Age ** 3)'])}\\,Age^{{3}}$"
)
...
plt.ylabel("AUC")
plt.legend()
plt.title(f"Cubic regression of AUC (N1) vs Age\n{formula_latex}", fontsize=12)
#plt.despine(bottom=True, left=True)
plt.show()
plt.savefig('figures/cubic_regression_auc_N1_age_b.svg', bbox_inches="tight")
