...
gibbs.full.low.rank.brr <- function(model, data, n.iter, thin, fixed.brr.rank=NULL,
                                    brr.vars.to.record=c('Gamma.local.shrinkage',
                                                         ...
                                                         'brr.rank', 'a3a4', 'latent.noise.var',
                                                         'Omega'),
                                    fa.vars.to.record=c('a1a2','rank', 'Lambda', 'variances'),
                                    brr.vars.to.fix=c('Psi.local.shrinkage','a3a4'),
                                    ind.struct.noise = FALSE,
  ...
  # generate n.iter samples from the posterior
	for (iter in 1:n.iter) {
		if((iter %% 10) == 0) {
			cat(".")
			if(iter%%100==0) cat(paste(iter,'brr rank', model$brr$context$brr.rank,'\n'))
		}
