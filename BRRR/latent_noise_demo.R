...
current.path <- '/projects/FABEEG/BRRR'
...
generative.model.and.data <- simulate.model.and.data(n.pheno=n.pheno, n.snps=n.snps, n.patients = (n.train+n.test), 
                                                     test.set.indexes = test.set.indexes, X.ptve = 0.03, 
                                                     res.var.ptve = 0.2, lambda = 0)
...
init.model$brr$context$Omega <- matrix(rnorm(brr.rank.init*n.train, sd = sqrt(init.model$brr$context$latent.noise.var)), 
...
set.seed(125)
mcmc.output <- gibbs.full.low.rank.brr(model=init.model, data=data, n.iter=n.iter, thin=thin, fixed.brr.rank=brr.rank, 
                                       fa.vars.to.record=c('variances','local.shrinkage','rank','a1a2','deltas'),
                                       brr.vars.to.fix=c('Gamma.local.shrinkage','a3a4'))
